...
int main()
{
    ...
    lgw_spi_r(spi_target, spi_mux_mode, LGW_SPI_MUX_TARGET_SX1301, 0x55, &data);
    printf("data received (simple read1): %d\n",data);
    
    data = 0;
    
    lgw_spi_r(spi_target, spi_mux_mode, LGW_SPI_MUX_TARGET_SX1301, 0xAA, &data);
    printf("data received (simple read1): %d\n",data);
    
    lgw_spi_rb(spi_target, spi_mux_mode, LGW_SPI_MUX_TARGET_SX1301, 0x55, datain, 16);
    for(i=0;i<16;i++)printf("data received (simple read3): %d\n",datain[i]);
